...
```
                    ...
                    └──────────┬──────────┘
                               ...
                               ▼
┌──────────────────┐    ┌─────────────────────┐    ┌──────────────────────┐
│  wheel_odom_node │───▶│   graph_optimizer   │───▶│ /aruco_slam/odom     │
...
| `wheel_odom_node` | `joint_states` → 휠 오도메트리 변환, `/aruco_slam/odom`과 동기화하여 odom→base_link TF 발행 |
...
저장 경로는 `slam_params.yaml`의 `landmarks_save_path`에 지정합니다. 워크스페이스 경로에 맞게 절대 경로를 수정하세요.
...
landmarks_save_path: "/path/to/map/landmarks_map.json"  # save_landmarks 저장 경로

# 프레임 (base_link_frame: base_link 또는 base_footprint)
...
# 카메라 외부 파라미터 (base_link → camera_depth_optical_frame)
...
| `/odometry/wheel_odom_correction` | `nav_msgs/Odometry` | 휠 오도메트리 보정 신호 (wheel_odom_node Re-propagation용) |
...
- `map` → `odom`: graph_optimizer가 발행 (최적화된 map 기준 pose)
- `odom` → `base_link`: wheel_odom_node가 발행 (휠 오도메트리 위치, `base_link_frame` 파라미터로 child frame 지정 가능)
...
├── launch/
│   └── aruco_slam.launch.py  # 메인 launch (Vision-only: ArUco + Wheel Odom)
├── map/
│   └── landmarks_map.json    # 랜드마크 맵 (localization용)
├── models/
│   └── aruco_box/            # Gazebo용 ArUco 박스 모델
├── msg/
│   ├── MarkerArray.msg
...
├── src/
│   ├── aruco_detector_node.cpp   # ArUco 검출 (RGB-D)
│   ├── graph_optimizer.cpp       # Factor Graph SLAM 백엔드
│   ├── wheel_odom_node.cpp       # 휠 오도메트리 + Re-propagation
│   └── imu_preintegration.cpp   # IMU 전처리 (선택 사용, launch에서 비활성)
