...
Gazebo 시뮬레이션 환경(8m×6m, ArUco 마커 10개, ID 11~20)에서 Mapping 모드로 2회 반복 실험을 수행하였다. 각 실험은 환경 내 2바퀴 주행(2-loop)으로 구성된다. Ground Truth는 Gazebo의 `/odom_gt`(모델 상태)를 사용하였으며, Umeyama SE(3) 정렬로 좌표계를 일치시켰다.

**평가 도구:** `evaluate_slam.py` (evo 기반, rosbag 후처리)

#### 3.8.1 실험 개요

| | Run 1 (exp9) | Run 2 (exp10) |
|---|---|---|
| **Bag** | `mapping_20260303_140347` | `mapping_20260303_140533` |
| **총 주행 거리** | 18.23 m | 15.77 m |
| **루프 수** | 2 (Loop 1: 13.87 m, Loop 2: 4.32 m) | 2 (Loop 1: 10.75 m, Loop 2: 4.99 m) |

#### 3.8.2 궤적 정확도 (EKF 최종 출력, 전체 구간)

| 지표 | Run 1 | Run 2 | **2회 평균** |
|---|---|---|---|
| **APE Trans RMSE** | **0.089 m** | 0.149 m | **0.119 m** |
| APE Trans Mean | 0.078 m | 0.134 m | 0.106 m |
| APE Trans Median | 0.070 m | 0.144 m | 0.107 m |
| APE Trans Std | 0.043 m | 0.064 m | 0.054 m |
| APE Trans Max | 0.266 m | 0.319 m | — |
| **APE Rot RMSE** | **2.00°** | 3.42° | **2.71°** |
| APE Rot Mean | 1.55° | 2.69° | 2.12° |
| APE Rot Max | 9.34° | 14.04° | — |
| RPE Δ=1m Trans | 0.080 m | 0.084 m | 0.082 m |
| RPE Δ=2m Trans | 0.097 m | 0.106 m | 0.101 m |
| RPE Δ=5m Trans | 0.131 m | 0.190 m | 0.161 m |
| **Drift Rate** | **0.88%** | 1.26% | **1.07%** |
| Endpoint Error | 0.161 m | 0.199 m | 0.180 m |
| Endpoint Yaw Error | 1.31° | 4.65° | 2.98° |

**분석:**

- **APE Trans RMSE 평균 0.119m** — 8m×6m 환경에서 약 12cm 수준의 위치 정확도를 달성하였다. Run 1(0.089m)이 Run 2(0.149m)보다 현저히 우수한데, 이는 주행 경로에 따른 마커 관측 빈도와 시야각 조건의 차이에 기인한다.
- **RPE Δ=1m에서 ~8cm**: 1m 이동 구간의 로컬 정확도가 8cm 수준으로, 경로 추종 제어에 충분한 정밀도이다. 두 실험 간 RPE Δ=1m 차이가 미미(0.080 vs 0.084)한 것은 단거리 상대 정확도가 마커 재관측과 무관하게 IMU+Wheel Odom에 의해 안정적으로 유지됨을 보여준다.
- **Drift Rate 평균 1.07%** — LIO-SAM 논문의 실외 LiDAR SLAM 0.5~2%와 유사한 수준이다. 마커 재관측에 의한 자동 loop closing이 drift를 효과적으로 억제하였다.
- **RPE가 Δ 증가에 따라 커지는 패턴**: Δ=1m(0.082m) → Δ=2m(0.101m) → Δ=5m(0.161m)으로 장거리 구간에서 오차가 누적되는 것은 SLAM 시스템의 일반적 특성이나, 증가율이 선형 이하로 억제되어 있다.

#### 3.8.3 루프별 분석

| 구간 | Run 1 APE RMSE | Run 2 APE RMSE | Run 1 Drift | Run 2 Drift |
|---|---|---|---|---|
| **Loop 1** | **0.081 m** | 0.139 m | **0.44%** | 1.03% |
| **Loop 2** | 0.109 m | 0.169 m | 3.72% | 4.00% |
| **Full** | 0.089 m | 0.149 m | 0.88% | 1.26% |

**분석:**

- **Loop 1이 Loop 2보다 일관되게 우수하다.** Loop 1의 APE RMSE 평균 0.110m vs Loop 2의 0.139m. Loop 1은 긴 주행 거리(10~14m)에서 다수의 마커를 순차 관측하며 그래프가 충분히 구축되므로, ISAM2 최적화가 효과적으로 동작한다.
- **Loop 2의 Drift Rate 급증(3.7~4.0%)**: Loop 2는 주행 거리가 짧고(4~5m) Loop 1에서 누적된 오차 위에 추가 주행하므로, 거리 대비 endpoint error 비율이 높게 산출된다. 이는 drift rate 지표의 구조적 한계이며, 절대 endpoint error(0.161~0.199m)는 두 루프 모두에서 20cm 이내로 양호하다.
- **Loop 1의 폐루프 오차**: Run 1에서 Loop 1 단독의 drift rate가 0.44%로 매우 낮다. 이는 첫 번째 루프 완주 시 모든 마커를 재관측하여 loop closing이 완전히 수행되었음을 의미한다.

#### 3.8.4 랜드마크 추정 정확도

Mapping 모드에서 ISAM2가 추정한 10개 랜드마크 위치를 Gazebo 배치 좌표(마커 면 기준)와 비교한 결과이다.

| 마커 ID | Run 1 (m) | Run 2 (m) | **평균 (m)** | 비고 |
|:---:|:---:|:---:|:---:|---|
| 11 | 0.130 | 0.238 | 0.184 | 환경 가장자리 |
| 12 | 0.154 | 0.217 | 0.185 | 환경 가장자리 |
| 13 | 0.045 | 0.082 | 0.063 | 경로 근접 |
| 14 | **0.039** | 0.107 | 0.073 | Run 1 최소 오차 |
| 15 | 0.045 | **0.076** | **0.060** | 평균 최소 오차 |
| 16 | 0.069 | 0.113 | 0.091 | |
| 17 | 0.055 | 0.192 | 0.124 | |
| 18 | 0.186 | 0.268 | 0.227 | 환경 가장자리 |
| 19 | 0.135 | **0.313** | 0.224 | 평균 최대 오차 |
| 20 | 0.128 | 0.301 | 0.215 | 환경 가장자리 |
| **평균** | **0.099** | **0.191** | **0.145** | |
| **최대** | 0.186 | 0.313 | — | |

**분석:**

- **Run 1의 평균 랜드마크 오차 9.9cm, Run 2는 19.1cm** — 두 실험 간 차이가 약 2배이다. Run 1이 더 긴 주행 거리(18.23m)로 각 마커를 더 다양한 시야각에서 관측하여 ISAM2가 보다 정확한 위치를 추정하였다.
- **지리적 패턴이 일관적이다**: 두 실험 모두에서 마커 13~15(로봇 주행 경로 근처)의 오차가 가장 낮고(4~11cm), 마커 18~20(환경 가장자리)의 오차가 가장 높다(13~31cm). 이는 가장자리 마커가 먼 거리에서만 관측되어 PnP 병진 추정 정확도가 떨어지는 것으로, §2.2.2의 동적 노이즈 모델 $\sigma_{\text{trans}}(d, \theta) = \sigma_0 \cdot (1 + 0.3d)$이 이 현상을 반영하고 있다.
- **평균 14.5cm**: 8m×6m 환경에서 약 1.5~2% 스케일의 랜드마크 위치 오차로, Localization 모드에서의 점유 격자 생성 및 경로 계획에 충분한 정밀도이다.

#### 3.8.5 궤적 시각화

각 실험의 EKF 궤적과 SLAM(ISAM2) 궤적을 Ground Truth와 비교한 플롯:

- **Run 1**: `results/exp9/trajectory_ekf.png`, `results/exp9/trajectory_slam.png`
- **Run 2**: `results/exp10/trajectory_ekf.png`, `results/exp10/trajectory_slam.png`

EKF 궤적이 SLAM 궤적 대비 더 부드럽고 GT에 밀착하는 경향이 관찰되며, 이는 §2.3에서 분석한 이중 추정기 아키텍처의 설계 의도(SLAM의 이산적 점프를 EKF가 평활화)를 시각적으로 확인시켜 준다.

#### 3.8.6 종합 요약

| 카테고리 | 지표 | 2회 평균 결과 | 평가 |
|---|---|---|---|
| **전역 정확도** | APE Trans RMSE | 0.119 m | 환경 크기의 ~1.5% |
| **로컬 정확도** | RPE Δ=1m Trans | 0.082 m | 제어에 충분 |
| **장기 안정성** | Drift Rate (Full) | 1.07% | LIO-SAM급 |
| **폐루프 일관성** | Endpoint Error | 0.180 m | 20cm 이내 복귀 |
| **랜드마크 매핑** | Mean Landmark Error | 0.145 m | 14.5cm 정확도 |
| **회전 정확도** | APE Rot RMSE | 2.71° | Heading 안정적 |
